...
function [parts,position,valueOut]=getMaxMask(I,scale_factor_zoom,mapH,mapV,maskpos,numparts,tol,treshold)
...
for i=1:numparts
    ...
    if(outOfRange==false)
        ...
        positionH(end+1,1:2)=[round(a),round(b)];
    end   
...
for i=1:numparts
    ...
    if(outOfRange==false)
        ...
        positionV(end+1,1:2)=[round(a),round(b)];
    end   
...
parts=[partsH,partsV];
%parts=partsV;
positionT=[positionH;positionV];
position(:,1)=positionT(:,2);
position(:,2)=positionT(:,1);
