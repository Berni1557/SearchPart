...
function [b] = colfiltQFP2(varargin)
    ...
    if all(block>=size(a)), % Process the whole matrix at once.
        ...
        for i=0:mblocks-1,
            for j=0:nblocks-1,
                ...
                T2=T1(14:end-14,11:end-11);
                %T1=reshape(T1,1,size(T1,1)*size(T1,2));
                %x = im2col(aa(i*mb+arows,j*nb+acols),nhood);
                [x,ind] = im2colpqfp(aa(i*mb+arows,j*nb+acols),nhood,T1);
                %xg = im2col(Icaa(i*mb+arows,j*nb+acols),nhood);
                [xc,ind] = im2colcqfp(Icaa(i*mb+arows,j*nb+acols,:),nhood,T1);
                %ind=find(T1);
                %x=x(:,ind);
                %xg=xg(:,ind);
