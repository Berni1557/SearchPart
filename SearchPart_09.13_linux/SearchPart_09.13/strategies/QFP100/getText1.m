...
function [textOut1,textOut2]=getText1(Wtext,bar)
    ...
    for im=1:size(Wtext,2)
        W=Wtext{im};
        %W=imrotate(W,90);
        cut=round((70*size(W,2))/463);         % compute margin
        W=W(cut:end-cut,cut:end-cut,:);       % cut margin
        pix=10;
        bar.update();
        ...
        for s=0.3:0.05:0.8
            ...
            %ind=(b(:,3)>b1-20) .* (b(:,4)>b2-30) .* (b(:,3)<b1+30) .* (b(:,4)<b2+30);
            if(~isempty(b))
                ind=(b(:,3)>10) .* (b(:,4)>10) .* (b(:,3)<b1+30) .* (b(:,4)<b2+30);
    ...
    [output11,output21]=OCRtesseract1(textOut,NumLinesOCR);
    % use neural network for second character recognition algorithm
    [output12,output22]=OCRtesseract2(textOut,NumLinesOCR);
    % marge results of both character recognition alorithms
     textOut1{end+1}=[output11,output12];
     textOut2{end+1}=[output21,output22];
    end
