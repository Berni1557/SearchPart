...
if kind(1)=='s', % Sliding
    if all(block>=size(a)), % Process the whole matrix at once.
        ...
        temp = feval(fun, chunk(:),chunk(:), params{:});
        ...
        Igaa=uint8(zeros(size(aa)));
        Igaa(floor((nhood(1)-1)/2)+(1:ma),floor((nhood(2)-1)/2)+(1:na))=Ig;
        ...
        for i=0:mblocks-1,
            for j=0:nblocks-1,
                ...
                T2=T1(6:end-6,22:end-22);
                %T1=reshape(T1,1,size(T1,1)*size(T1,2));
                %x = im2col(aa(i*mb+arows,j*nb+acols),nhood);
                [x,ind] = myim2col(aa(i*mb+arows,j*nb+acols),nhood,T1);
                %xg = im2col(Igaa(i*mb+arows,j*nb+acols),nhood);
                [xg,ind] = myim2col(Igaa(i*mb+arows,j*nb+acols),nhood,T1);
                %ind=find(T1);
                %x=x(:,ind);
                %xg=xg(:,ind);
                
                out=feval(fun,x,xg,params{:});
