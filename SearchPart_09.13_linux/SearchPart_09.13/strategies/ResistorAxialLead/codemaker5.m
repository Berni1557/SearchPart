...
function [value,tolerance,codeName,isr]=codemaker5(code)
%Inputs:
...
codetemp=[];
    ...
    for i=1:4
        if(code(i)==11)                     % if last ring is not golden or silver, part is not a resistor and isr is false
            isr=false;
        end
    end
    
    codeName=[colorName{code(1)},', ',colorName{code(2)},', ',colorName{code(3)},', ',colorName{code(4)},', ',colorName{code(5)}];
    ...
    switch code(5)              % get value of fifth ring
        case 11
            tolerance=5;
        case 12
            tolerance=10;
