...
function b = colfiltDIP1c(varargin)
...
if kind(1)=='s', % Sliding
    if all(block>=[size(a,1),size(a,2)]), % Process the whole matrix at once.
        % Expand A
        [ma,na] = size(a);
        ...
        if indexed && isa(a, 'double'),
            ...
            a2=a(:,:,2);
            a3=a(:,:,3);
            aa1 = mymkconstarray(class(a1), 0, size(a1) + [mpad npad] + (nhood-1));
            aa2 = mymkconstarray(class(a2), 0, size(a2) + [mpad npad] + (nhood-1));
            aa3 = mymkconstarray(class(a3), 0, size(a3) + [mpad npad] + (nhood-1));
            aa=cat(3,aa1,aa2);
            aa=cat(3,aa,aa3);
        ...
        for i=0:mblocks-1,
            for j=0:nblocks-1,
                ...
                x1 = im2col(aa(i*mb+arows,j*nb+acols,1),nhood);
                x2 = im2col(aa(i*mb+arows,j*nb+acols,2),nhood);
                x3 = im2col(aa(i*mb+arows,j*nb+acols,3),nhood);
                x=[x1;x2;x3];
                b(i*mb+brows,j*nb+bcols) = reshape(feval(fun,x,params{:}),block(1),block(2));
            end
        end
        b = b(1:ma,1:na);
    end
